...
def plot_layer_sepreate(local_IXT_CAS,local_ITY_CAS,finallocal_IXT_CAS,finallocal_ITY_CAS,structure,num_of_experment):
        ...
        for layernum in range(len(structure)):
            ax=fig1.add_subplot(2,int(np.ceil(len(structure)/2)),layernum+1)
...
#========================load======================= 
os.chdir('D:\\Information-Cascade-master\\Synthetic datasets\\Cascade structure')    
for number_of_data in [17, 28, 44, 48]:
    for num_of_train in range(1):
        ...
        for layernum in range(len(structure)):
            axes.plot(hISTORY['iter_TRAIN'+str(layernum)][0][0],label='Layer '+str(layernum)+', Average:'+str(round(np.mean(hISTORY['iter_TRAIN'+str(layernum)][0][0]),3)))           
        ...
        plo.finalplotfigure(local_IXT_CAS_array,local_ITY_CAS_array,epochsInds,information_figure_path)
